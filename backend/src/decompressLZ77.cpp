...
NAN_METHOD(compress_LZ77)
{
    ...
    info.GetReturnValue().Set(Nan::New<String>(output.c_str()).ToLocalChecked());
}

NAN_METHOD(decompress_LZ77)
{
    Nan::HandleScope scope;
    Isolate *isolate = info.GetIsolate();
    String::Utf8Value cmd(isolate, info[0]);
...
NAN_MODULE_INIT(init) {
    // Nan::SetMethod(target, "compress_LZ77", compress_LZ77);
    Nan::SetMethod(target, "decompress_LZ77", decompress_LZ77);
}

// NODE_MODULE(compress_LZ77, init)
NODE_MODULE(decompress_LZ77, init)